...
<%
    String ret = "";
    try {
        String addr = "https://dev2.coocon.co.kr:8443/sol/gateway/ins_wapi_mobile.jsp";
        String param= request.getParameter("JSONData");
        
        
        //System.out.println("Addr(IN): "+addr);
        //System.out.println("Param(IN): "+param);
        if(param==null) {
            ret="{\"RESULT_CD\":\"-1\",\"RESULT_MG\":\"No Input\"}";
        } else {
            //addr = URLDecoder.decode(addr);
            param = "JSONData=" + URLEncoder.encode(URLEncoder.encode(param,"UTF-8"),"UTF-8");
            out.clear();
            if(!"".equals(param)) {
